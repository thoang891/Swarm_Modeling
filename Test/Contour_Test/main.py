...
def surf_plot(surface_plot, contour_plot):
    ...
    contour = ax.contour(env.x_space, env.y_space, env.z_space, zdir='z', offset=0, cmap='viridis', alpha=0.5)
    ax.clabel(contour, inline=True, fontsize=8, fmt='%1.1f')  # Label contour lines with Z value

    return surface_plot, contour
...
def main(iters=set.settings['iterations'], log_folder=None):
    swarm.construct()
    if set.settings['animation']:
        surface_plot, contour = surf_plot(surface_plot, contour)
        scatter_plots = []
