...
class Env():
    ...
    @staticmethod
    def scalar(x, y): 
        bounds = set.settings['map_size']
        
        # z = np.sinc((x/5)**2 + (y/5)**2) + np.sinc((x + 2)/5 + (y + 2)/5)/2
        # z = np.cos(x/2) + np.sin(y/2)
        # z = -((x)**2 + (y)**2)
        # z = x**2 + 20*np.sin(x) + y**2 - 20*np.sin(y)
        z = 1.5 * np.exp(-(x**2 + y**2) / (2*4**2))
        z += 0.5 * np.exp(-((x+10)**2 + (y+10)**2) / (2*4**2))
        z += 0.75 * np.exp(-((x-10)**2 + (y-10)**2) / (2*4**2))
        z += np.exp(-((x-10)**2 + (y+10)**2) / (2*4**2))
        z += np.exp(-((x+10)**2 + (y-10)**2) / (2*4**2))
        # for peaks in range(5):
        # z += random.uniform(0,1) * np.exp(-((x+random.uniform(-bounds, bounds))**2 + (y+random.uniform(-bounds,bounds))**2) / (2*4**2)) 
    ...
    def update_scalar(self):
        ...
        self.scalar = lambda x, y: np.exp(-((x-tar_pos_x)**2 + (y-tar_pos_y)**2) / (2*4**2))
