...
def analyze_swarm(folder_path, buoy_log, settings):
    ...
    swarm_df = pd.DataFrame(columns=['Time','best_x_avg', 'best_y_avg', 'target_x', 'target_y', 'error_x', 'error_y', 'D_error', 'Time/Total Time', 'Accuracy', 'Battery', 'Non-Dimensional Battery'])
    ...
    battery_nom = 47520
    seeker_battery = float(settings_df[settings_df['Setting'] == 'seeker_battery_number']['Value'].values[0]) * battery_nom
    seeker_population = float(settings_df[settings_df['Setting'] == 'seeker_population']['Value'].values[0])

    explorer_battery = float(settings_df[settings_df['Setting'] == 'explorer_battery_number']['Value'].values[0]) * battery_nom
    explorer_population = float(settings_df[settings_df['Setting'] == 'explorer_population']['Value'].values[0])

    isocontour_battery = float(settings_df[settings_df['Setting'] == 'iso_battery_number']['Value'].values[0]) * battery_nom
    isocontour_population = float(settings_df[settings_df['Setting'] == 'isocontour_population']['Value'].values[0])

    total_battery = ((seeker_battery * seeker_population) + (explorer_battery * explorer_population) + (isocontour_battery * isocontour_population))/(3600) # Convert to w*h from joules
...
def analyze_seekers(folder_path, buoy_log, settings):
    ...
    Speed_Seeker = float(settings_df[settings_df['Setting'] == 'seeker_speed_number']['Value'].values[0]) # Speed number of seekers
    Speed_Target = float(settings_df[settings_df['Setting'] == 'target_speed_number']['Value'].values[0]) # Speed number of target
    ...
    seeker_df['Non-Dimensional Target Proximity'] = (seeker_df['Distance to Target'] / np.sqrt(bounded_area)).round(3)
    ...
    seeker_df['Heading-Bearing Correlation'] = (seeker_df['error_x_norm']*seeker_df['u'] + seeker_df['error_y_norm']*seeker_df['v'])/magnitude.round(3)
    ...
    plt.text(0.5, Proximity_Threshold + 0.05, 'Proximity Threshold', horizontalalignment='center', verticalalignment='center', color='r')
    ...
    if settings_df[settings_df['Setting'] == 'target_setting']['Value'].values[0] == 'ON':
        hist, bins, _ = plt.hist(seeker_df['Heading-Bearing Correlation'], bins=10, alpha=0.5, label='Heading-Bearing Correlation')
...
def analyze_coverage(folder_path, buoy_log, settings):
    ...
    map_size_value = float(settings_df[settings_df['Setting'] == 'map_size']['Value'].values[0])
    pop = int(settings_df[settings_df['Setting'] == 'seeker_population']['Value'].values[0]) + int(settings_df[settings_df['Setting'] == 'explorer_population']['Value'].values[0]) + int(settings_df[settings_df['Setting'] == 'isocontour_population']['Value'].values[0])
...
def analyze_isocontours(folder_path, buoy_log, settings):
    ...
    isocontour_performance_df['Sum Linear Coverage/Perimeter'] = (isocontour_performance_df['Sum Linear Coverage'] / isocontour_performance_df['Perimeter']).round(3)
