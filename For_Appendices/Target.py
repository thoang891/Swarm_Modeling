...
class Target():
    ...
    def __init__(self, timestep = 0.1, bounds = 10, speed_number=1, inertia=0.5):
        ...
        self.target_data = {'ID': self.ID, 'behv': "Target", 'x': round(self.position[0], 2), 'y': round(self.position[1], 2), 'u': None, 'v': None}
    ...
    def move(self, env):
        ...
        self.position[0] += (self.inertia * self.prev_velocity[0] + (1-self.inertia) * self.velocity[0] + external_force[0])*self.dt
        self.position[1] += (self.inertia * self.prev_velocity[1] + (1-self.inertia) * self.velocity[1] + external_force[1])*self.dt
    ...
    def motor(self):
        ...
        self.repulse()
        self.circle()
        self.velocity = [self.speed * (self.A * self.random_vector[0] + self.B * self.repulsion_vector[0] + self.C * self.circle_vector[0]), self.speed * (self.A * self.random_vector[1] + self.B * self.repulsion_vector[1] + self.C * self.circle_vector[1])]
    ...
    def circle(self):
        ...
        if magnitude != 0:
            self.circle_vector = [circle_vector[0]/magnitude, circle_vector[1]/magnitude]
    ...
    def update(self, env):
        ...
        self.move(env)
        self.measure(env)
        self.target_data = {'ID': self.ID, 'x': round(self.position[0], 2), 'y': round(self.position[1], 2), 'u': round(self.velocity[0], 2), 'v': round(self.velocity[1], 2), 'Measurement': round(self.measurement, 3)}
