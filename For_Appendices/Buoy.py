...
class Buoy():
    ...
    def __init__(self, id, behv="seeker", speed=2, com_radius=7, repulsion_radius=0.5, iso_thresh=5, iso_seek_rad=0.4, iso_spread_rad=4.5, iso_goal=50, battery=47520, env=None, inertia=0.5):
        self.id = id
        self.env = env
        ...
        self.isocontour_goal = iso_goal # Goal measurement 
    ...
    def update_battery(self):
        '''
        Calculate battery discharge based on magnitude of velocity developed 
        by motor function.
        Velocity is translated into power based on a cubic polynomial fit of 
        buoy drag properties developed by Brandon Zoss in his thesis. 
        Power is then integrated over the time step to calculate the discharge. 
        Discharge is then subtracted from the battery level.
        '''
        ...
        if self.velocity is not None:
            ...
            # Rounded to 2 decimal places to prevent negative power values
            velocity_magnitude = round(np.linalg.norm(self.velocity), 2)
    ...
    def behavior(self):
        ...
        print("Buoy {0} is a {1} with weights A: {2}, B: {3}, C: {4}, D: {5}, E: {6}".format(self.id, self.behv, self.A, self.B, self.C, self.D, self.E))
    ...
    def move(self):
        ...
        self.position[0] += (self.inertia * self.prev_velocity[0] + (1-self.inertia) * self.velocity[0] + F[0]) * self.env.dt
        self.position[1] += (self.inertia * self.prev_velocity[1] + (1-self.inertia) * self.velocity[1] + F[1]) * self.env.dt
    ...
    def motor(self):
        ...
        # Define behavior vectors
            ...
            u = self.speed*(self.A*self.local_goal_vector[0] + self.B*self.global_goal_vector[0] + self.C*self.repulsion_vector[0] + self.D*self.random_vector[0] + self.E*self.isocontour_vector[0])
            v = self.speed*(self.A*self.local_goal_vector[1] + self.B*self.global_goal_vector[1] + self.C*self.repulsion_vector[1] + self.D*self.random_vector[1] + self.E*self.isocontour_vector[1])
    ...
    def local_goal(self):
        # Copy the broadcast data to a local variable
        ...
        if data_frame:
            ...
            # Move towards the buoy with the maximum measurement if the current
            # buoy's measurement is less than the maximum measurement
    ...
    def global_goal(self):
        ...
        distance = np.sqrt((self.best_known_position[0] - self.position[0])**2 + (self.best_known_position[1] - self.position[1])**2)
        
        if distance > 0.001:
            goal_vector_unnormalized = [self.best_known_position[0] - self.position[0], self.best_known_position[1] - self.position[1]]
    ...
    def isocontour_walk(self):
        ...
        # Repulsion radius when searching for the contour goal
        seeking_repulsion_rad = self.isocontour_seeking_repulsion_radius
        # Repulsion radius when within contour goal threshold to promote spreading
        spreading_repulsion_rad = self.isocontour_spreading_repulsion_radius
        ...
        # Check if the self measurement is less than goal contour
            ...
            for data in data_frame:
                if data['Measurement'] > self.measurement:
                    ...
                    sum_neighbor_vector[0] += neighbor_vector_unnormalized[0]/weight
                    sum_neighbor_vector[1] += neighbor_vector_unnormalized[1]/weight
                    print("Adding neighbor vector: {0} from buoy {1} because it is measuring {2}".format(
                        neighbor_vector_unnormalized, data['ID'], data['Measurement']))
                ...
                if data['ID'] != self.id:
                    if data['Battery'] > 0:
                        ...
                        if distance <= data['com_radius']:
                            ...
                            print("Unable to read buoy {0}'s data because the distance {1:>.2f} is greater than the communication radius {2}".format(data['ID'], distance, data['com_radius']))
                    else:
                        print("Unable to read buoy {0}'s data because buoy {0} is out of battery".format(data['ID']))

            print("The remaining neighbor data for buoy {} is:".format(self.id))
             # Print broadcast data to be read by buoy
            print("\n".join(str(data) for data in self.broadcast_data_processed))
    ...
    def memory(self):
        ...
        if data_frame:
           ...
           # If neighbor has a better measurement, 
           # update best known position and measurement
            ...
            # If neighbor has a better best known measurement, 
            # update best known position and measurement
        ...
        # If own measurement is better than best known measurement, 
        # update best known position and measurement
    ...
    def count_neighbors(self):
        ...
        print("Buoy {0} has {1} neighbors, {2} seekers, {3} explorers, and {4} isocontours".format(self.id, self.N, self.Ns, self.Ne, self.Ni))
