...
# Objective Function
def fx(x, y):
    return (np.sinc(x) + np.sinc(y) + (np.cos(x * y) / 10))

#Obectjive Function for Swarm

def objective_func(x): # Known solution at position (0,0) and value of 2
    x_ = x[:, 0]
    y_ = x[:, 1]
    z = fx(x_, y_)
    return z
...
for i in range(iters):
    ...
    for particle_index in range(n_particles):
        ...
        fx(x, y)
        # z = (np.sinc(x) + np.sinc(y) + (np.cos(x * y) / 10)) * -1
